...
public class LabelDesignerView extends View {
    ...
    private Rect detectProductBounds(Bitmap bmp, int sensitivity, float paddingPct) {
        ProductDetectionEngine.Result result = ProductDetectionEngine.detect(bmp, sensitivity, paddingPct);
        smartSnapConfidence = result == null ? 0f : result.confidence;
        return result == null || result.rect == null ? null : new Rect(result.rect);
    }
