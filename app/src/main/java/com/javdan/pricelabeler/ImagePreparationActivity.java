...
public class ImagePreparationActivity extends Activity {
 ...
 void smartSnap(){
  int w=working.getWidth(),h=working.getHeight();
  ProductDetectionEngine.Result d=ProductDetectionEngine.detect(working,sensitivity.getProgress(),padding.getProgress()/100f);
  if(d==null||d.rect==null){
    // Never dead-end: provide a conservative editable proposal.
    view.setCropNormalized(new RectF(.08f,.08f,.92f,.92f));
    info.setText("تشخیص قطعی نبود؛ کادر پیشنهادی ساخته شد. گوشه‌ها را اصلاح کنید و سپس Crop را بزنید.");
    Toast.makeText(this,"تشخیص قطعی نبود؛ کادر پیشنهادی قابل ویرایش نمایش داده شد",Toast.LENGTH_LONG).show();
    return;
  }
  Rect r=d.rect;
  view.setCropNormalized(new RectF(r.left/(float)w,r.top/(float)h,r.right/(float)w,r.bottom/(float)h));
  int conf=Math.round(d.confidence*100f);
  info.setText("Smart Snap: "+conf+"٪ — کادر را بررسی/اصلاح کنید و سپس Crop را بزنید.");
 }
