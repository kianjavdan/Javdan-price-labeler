...
public class ImagePreparationActivity extends Activity {
 ...
 void accept(){try{File d=new File(getCacheDir(),"prepared");d.mkdirs();File f=new File(d,"prepared_"+System.currentTimeMillis()+".jpg");try(FileOutputStream o=new FileOutputStream(f)){working.compress(Bitmap.CompressFormat.JPEG,96,o);}Intent i=new Intent();i.putExtra(EXTRA_PREPARED_PATH,f.getAbsolutePath());setResult(RESULT_OK,i);finish();}catch(Exception e){Toast.makeText(this,"ذخیره تصویر ناموفق بود: "+e.getMessage(),Toast.LENGTH_LONG).show();}}
