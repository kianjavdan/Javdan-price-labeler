...
public class MainActivity extends Activity {
    ...
    static final int PICK_EXCEL = 10, PICK_IMAGE = 11, PICK_FOLDER = 12, PICK_BACKGROUND = 13, TAKE_PHOTO = 14, PREPARE_IMAGE = 15;
    ...
    private void launchImagePreparation(String path){
        try {
            Intent i = new Intent(this, ImagePreparationActivity.class);
            i.putExtra(ImagePreparationActivity.EXTRA_SOURCE_PATH, path);
            startActivityForResult(i, PREPARE_IMAGE);
        } catch (Exception e){
            Toast.makeText(this,"خطا در باز کردن آماده‌سازی تصویر: "+safeMessage(e),Toast.LENGTH_LONG).show();
        }
    }

    private File copyUriForPreparation(Uri uri) throws Exception {
        File dir = new File(getCacheDir(), "image_prepare");
        if (!dir.exists() && !dir.mkdirs()) throw new IOException("پوشه آماده‌سازی ساخته نشد");
        File out = new File(dir, "source_"+System.currentTimeMillis()+".jpg");
        try (InputStream in = getContentResolver().openInputStream(uri);
             FileOutputStream fos = new FileOutputStream(out)) {
            if (in == null) throw new IOException("تصویر باز نشد");
    ...
    protected void onActivityResult(int req, int result, Intent data){
        ...
        if (req == PREPARE_IMAGE){
            if (result == RESULT_OK && data != null){
                String path = data.getStringExtra(ImagePreparationActivity.EXTRA_PREPARED_PATH);
                if (path != null && importCameraPhoto(new File(path))) return;
                Toast.makeText(this,"تصویر آماده‌شده قابل دریافت نبود",Toast.LENGTH_LONG).show();
            }
            return;
        }
        ...
        try {
            if (req == PICK_IMAGE){
                File prep = copyUriForPreparation(u);
                launchImagePreparation(prep.getAbsolutePath());
                return;
