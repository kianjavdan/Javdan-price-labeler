...
public class MainActivity extends Activity {
    ...
    LinearLayout body, manualRows, excelInfo;
    ...
    private TextView tv(String s,int sp,boolean bold){
        TextView t=new TextView(this); t.setText(s); t.setTextSize(sp); t.setTextColor(0xFF222222);
    ...
    private void buildUi(){
        ...
        root.addView(tv("نسخه اندروید — Excel + ورود دستی قیمت + طراح Drag & Drop",14,false));

        LinearLayout tabs=row();
        tabData=btn("Excel / دستی"); tabDesigner=btn("طراح برچسب"); tabOutput=btn("خروجی");
        tabs.addView(tabData,new LinearLayout.LayoutParams(0,-2,1)); tabs.addView(tabDesigner,new LinearLayout.LayoutParams(0,-2,1)); tabs.addView(tabOutput,new LinearLayout.LayoutParams(0,-2,1));
        root.addView(tabs);
        ScrollView sv=new ScrollView(this); body=new LinearLayout(this); body.setOrientation(LinearLayout.VERTICAL); body.setPadding(4,12,4,80); sv.addView(body); root.addView(sv,new LinearLayout.LayoutParams(-1,0,1));
        status=tv("آفلاین • بدون آپلود • عکس اصلی تغییر نمی‌کند",12,false); root.addView(status);
        setContentView(root);
    ...
    private void renderDataContent(boolean excel){
        ...
        if(excel){
            Button pe=btn("انتخاب فایل Excel (.xlsx)"); body.addView(pe);
            ...
            excelInfo=new LinearLayout(this); excelInfo.setOrientation(LinearLayout.VERTICAL); body.addView(excelInfo);
            if(!headers.isEmpty()) refreshExcelUi();
            Button validate=btn("بررسی تطبیق Excel و تصاویر"); body.addView(validate);
            validate.setOnClickListener(v->validateBatch());
        }else{
            body.addView(tv("حالت دستی",18,true));
            Button pi=btn("انتخاب / دراپ عکس از گالری یا Files"); body.addView(pi); pi.setOnClickListener(v->pickFile("image/*",PICK_IMAGE));
            rialToToman=new CheckBox(this); rialToToman.setText("قیمت‌های واردشده ریال هستند؛ تبدیل به تومان ÷۱۰"); body.addView(rialToToman);
            body.addView(tv("نام و قیمت هر ردیف را خودت می‌توانی تغییر بدهی:",14,false));
            ...
            rebuildManualRows();
            Button add=btn("+ افزودن قیمت جدید"); body.addView(add);
            add.setOnClickListener(v->{ syncManualRows(); fields.add(new LabelField("قیمت جدید","")); rebuildManualRows(); });
            Button go=btn("ذخیره قیمت‌ها و رفتن به طراح"); body.addView(go);
            go.setOnClickListener(v->{syncManualRows(); showDesigner();});
        }
    }

    private void makeDefaults(){
        fields.add(new LabelField("قیمت مصرف کننده",""));
        fields.add(new LabelField("قیمت پایه",""));
        fields.add(new LabelField("قیمت شماره ۱",""));
        fields.add(new LabelField("قیمت شماره ۲",""));
        relayoutFields();
    ...
    private void rebuildManualRows(){
        ...
        for(int i=0;i<fields.size();i++){
            ...
            EditText v=new EditText(this); v.setText(f.value); v.setHint("قیمت"); v.setInputType(2);
            ...
            r.addView(n,new LinearLayout.LayoutParams(0,-2,1.4f));r.addView(v,new LinearLayout.LayoutParams(0,-2,1));r.addView(del,new LinearLayout.LayoutParams(-2,-2));
            manualRows.addView(r); nameEdits.add(n); valueEdits.add(v);
        }
    }
    ...
    private String formatPrice(String raw,boolean rial){
        ...
        try{
            double d=Double.parseDouble(raw); if(rial)d/=10.0; long n=Math.round(d);
    ...
    private void relayoutFields(){
        ...
        for(int i=0;i<fields.size();i++){
            LabelField f=fields.get(i); f.x=.06f;f.w=.88f;f.h=Math.min(.18f,.78f/n);
            f.y=.06f+i*(.88f/n);
        }
    }
    ...
    private void showDesigner(){
        ...
        clear();
        body.addView(tv("طراح Drag & Drop",18,true));
        body.addView(tv("هر کادر قیمت را با انگشت بگیر و جابه‌جا کن.",13,false));
        ...
        appendMode=new CheckBox(this);appendMode.setText("کادر بیرون عکس");opts.addView(appendMode);
        labelWidth=new EditText(this);labelWidth.setHint("عرض %");labelWidth.setText("38");labelWidth.setInputType(2);opts.addView(labelWidth);
        body.addView(opts);
        Button add=btn("+ افزودن فیلد قیمت");body.addView(add);add.setOnClickListener(v->{fields.add(new LabelField("قیمت جدید",""));relayoutFields();designer.setFields(fields);saveTemplate();});
        ...
        Button sample=btn("انتخاب عکس نمونه");body.addView(sample);sample.setOnClickListener(v->pickFile("image/*",PICK_IMAGE));
        body.addView(tv("تنظیمات فیلد انتخابی:",14,true));
        fieldEditorContainer=new LinearLayout(this);fieldEditorContainer.setOrientation(LinearLayout.VERTICAL);body.addView(fieldEditorContainer);
    }
    ...
    private void showFieldEditor(){
        ...
        EditText name=new EditText(this);name.setText(f.name);fieldEditorContainer.addView(name);
        EditText val=new EditText(this);val.setText(f.value);val.setHint("قیمت / مقدار");fieldEditorContainer.addView(val);
        CheckBox strike=new CheckBox(this);strike.setText("خط‌خورده کردن قیمت");strike.setChecked(f.strike);fieldEditorContainer.addView(strike);
        Button apply=btn("اعمال");fieldEditorContainer.addView(apply);
        apply.setOnClickListener(v->{f.name=name.getText().toString();f.value=val.getText().toString();f.strike=strike.isChecked();designer.invalidate();saveTemplate();});
        Button del=btn("حذف این فیلد");fieldEditorContainer.addView(del);
        del.setOnClickListener(v->{fields.remove(selectedField);selectedField=-1;relayoutFields();designer.setFields(fields);fieldEditorContainer.removeAllViews();saveTemplate();});
    }

    private void showOutput(){
        if(manualRows!=null && manualRows.getParent()!=null)syncManualRows();
        clear();
        body.addView(tv("خروجی",18,true));
    ...
    private void makePreview(){
        if(currentBitmap==null){Toast.makeText(this,"عکس انتخاب نشده",Toast.LENGTH_SHORT).show();return;}
        LabelDesignerView renderer=new LabelDesignerView(this);renderer.setFields(fields);
        boolean append=appendMode!=null&&appendMode.isChecked();
        float width=.38f;try{width=Float.parseFloat(labelWidth==null?"38":labelWidth.getText().toString())/100f;}catch(Exception ignored){}
        Bitmap out=renderer.renderFinal(currentBitmap,Color.WHITE,0xFFD8D8D8,append,width);
        ImageView iv=new ImageView(this);iv.setAdjustViewBounds(true);iv.setImageBitmap(out);
    ...
    private void saveCurrent(){
        ...
        String name="Javdan_"+System.currentTimeMillis()+".jpg";
        try{saveToGallery(out,name);Toast.makeText(this,"در Pictures/JavdanPriceLabeler ذخیره شد",Toast.LENGTH_LONG).show();}
    ...
    private void saveToGallery(Bitmap bmp,String name)throws Exception{
        ...
        try(OutputStream o=getContentResolver().openOutputStream(u)){bmp.compress(Bitmap.CompressFormat.JPEG,94,o);}
    }

    private void pickFile(String type,int request){
        Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType(type);startActivityForResult(i,request);
    }
    ...
    @Override protected void onActivityResult(int req,int result,Intent data){
        ...
        try{
            if(req==PICK_IMAGE){
                imageUri=u;currentBitmap=BitmapFactory.decodeStream(getContentResolver().openInputStream(u));
                status.setText("عکس انتخاب شد");
                if(designer!=null)designer.setProductBitmap(currentBitmap);
            }else if(req==PICK_EXCEL){
                excelUri=u;status.setText("در حال خواندن Excel...");
                excelRows=new XlsxReader(this).readFirstSheet(u);
                headers.clear();if(!excelRows.isEmpty())headers.addAll(excelRows.get(0).keySet());
                refreshExcelUi();status.setText(excelRows.size()+" ردیف Excel خوانده شد");
            }else if(req==PICK_FOLDER){
                folderUri=u;status.setText("پوشه تصاویر انتخاب شد");
            }
        }catch(Exception e){Toast.makeText(this,"خطا: "+e.getMessage(),Toast.LENGTH_LONG).show();}
    }

    private void refreshExcelUi(){
        if(codeSpinner!=null){ArrayAdapter<String>a=new ArrayAdapter<>(this,android.R.layout.simple_spinner_dropdown_item,headers);codeSpinner.setAdapter(a);}
        if(excelInfo!=null){excelInfo.removeAllViews();excelInfo.addView(tv("ردیف‌ها: "+excelRows.size()+" | ستون‌ها: "+headers.size(),13,false));}
    }

    private void validateBatch(){
        if(excelUri==null||folderUri==null||excelRows.isEmpty()){Toast.makeText(this,"Excel و پوشه عکس‌ها را انتخاب کن",Toast.LENGTH_LONG).show();return;}
        Toast.makeText(this,"آماده پردازش گروهی؛ کد از ستون انتخاب‌شده خوانده می‌شود.",Toast.LENGTH_LONG).show();
    }

    private void runBatch(){
        if(excelRows.isEmpty()||folderUri==null){Toast.makeText(this,"Excel و پوشه عکس‌ها را انتخاب کن",Toast.LENGTH_LONG).show();return;}
        new Thread(()->{
            int ok=0,missing=0,errors=0;
            try{
                String codeHeader=headers.get(codeSpinner!=null?codeSpinner.getSelectedItemPosition():0);
                HashMap<String,Uri> images=listTreeImages(folderUri);
                for(LinkedHashMap<String,String> row:excelRows){
                    String code=normalizeCode(row.get(codeHeader)); if(code.isEmpty())continue;
                    Uri img=images.get(code);
                    if(img==null){missing++;continue;}
                    try{
                        Bitmap src=BitmapFactory.decodeStream(getContentResolver().openInputStream(img));
                        ArrayList<LabelField> fs=new ArrayList<>();
                        int index=0;
                        for(String h:headers){
                            if(h.equals(codeHeader))continue;
                            String raw=row.get(h);if(raw==null||raw.trim().isEmpty())continue;
                            LabelField f=new LabelField(h,formatPrice(raw,rialToToman==null||rialToToman.isChecked()));
                            if(index<fields.size()){LabelField t=fields.get(index);f.x=t.x;f.y=t.y;f.w=t.w;f.h=t.h;f.strike=t.strike;f.titleColor=t.titleColor;f.priceColor=t.priceColor;f.titleSize=t.titleSize;f.priceSize=t.priceSize;}
                            fs.add(f);index++;
                        }
                        Bitmap out=render(src,fs);saveToGallery(out,code+".jpg");ok++;
                    }catch(Exception e){errors++;}
                }
            }catch(Exception e){errors++;}
            int fok=ok,fm=missing,fe=errors;
            runOnUiThread(()->Toast.makeText(this,"تمام شد — موفق: "+fok+" | بدون عکس: "+fm+" | خطا: "+fe,Toast.LENGTH_LONG).show());
        }).start();
    }
        ...
        try(Cursor c=getContentResolver().query(children,cols,null,null,null)){
            if(c!=null)while(c.moveToNext()){
                ...
                if(mime!=null&&mime.startsWith("image/")){
                    String stem=name;int dot=stem.lastIndexOf('.');if(dot>0)stem=stem.substring(0,dot);
                    Uri child=DocumentsContract.buildDocumentUriUsingTree(tree,id);map.put(normalizeCode(stem),child);
                }
            }
        }
        return map;
    ...
    private String normalizeCode(String s){
        if(s==null)return "";s=s.trim();if(s.endsWith(".0"))s=s.substring(0,s.length()-2);return s;
    }

    private void saveTemplate(){
        try{
            JSONArray a=new JSONArray();for(LabelField f:fields)a.put(f.toJson());
            getSharedPreferences("javdan",MODE_PRIVATE).edit().putString("fields",a.toString()).apply();
        }catch(Exception ignored){}
    }
    private void loadTemplate(){
        fields.clear();
        String s=getSharedPreferences("javdan",MODE_PRIVATE).getString("fields","");
        if(!s.isEmpty())try{JSONArray a=new JSONArray(s);for(int i=0;i<a.length();i++)fields.add(LabelField.fromJson(a.getJSONObject(i)));}catch(Exception ignored){}
        if(fields.isEmpty())makeDefaults();
    }
}
